...
const severityTone: Record<string, string> = {
  adequate: "border-brand-200 bg-brand-50 text-brand-700",
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-brand-700">Participant report</p>
      ...
      {participant.strengths.length > 0 && (
        <section>
          <h4 className="flex items-center gap-2 text-sm font-bold text-ink"><CheckCircle2 className="h-4 w-4 text-brand-600" aria-hidden="true" />What this person is doing well</h4>
      ...
      {(participant.gaps.some((item) => item.explanation) || participant.gaps.some((item) => item.recommendations.length > 0)) && (
        <section className="grid gap-4 md:grid-cols-2">
          <div>
            <h4 className="flex items-center gap-2 text-sm font-bold text-ink"><AlertTriangle className="h-4 w-4 text-accent-600" aria-hidden="true" />Why might this be happening?</h4>
          ...
          <div>
            <h4 className="flex items-center gap-2 text-sm font-bold text-ink"><ShieldCheck className="h-4 w-4 text-brand-600" aria-hidden="true" />Data-based suggestions</h4>
  ...
  return (
    <Card className="mt-6">
      <CardBody className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="text-xs font-bold uppercase tracking-wide text-brand-700">Nutrition gap analysis</p><h2 className="mt-1 text-xl font-bold text-ink">What the uploaded data shows</h2><p className="mt-1 text-sm text-muted">Actual records only. Missing reference targets remain clearly marked as not assessable.</p></div>
...
function Metric({ label, value, detail }: { label: string; value: string | number; detail?: string }) {
  return <div className="rounded-[10px] border border-line bg-white p-3"><p className="text-xs text-muted">{label}</p><p className="mt-1 break-words text-lg font-bold text-ink">{value}</p>{detail && <p className="mt-1 text-[11px] text-muted">{detail}</p>}</div>;
}
