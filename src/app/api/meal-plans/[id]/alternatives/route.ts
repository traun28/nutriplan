...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    ...
    return Response.json({ alternatives: alternativesFor(plan.data, profile, dayIndex, slot, 6, await pantryIngredientsFor(user.id, plan.data.options.preferPantry === true)) });
  } catch (error) {
